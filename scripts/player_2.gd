...
@export var down_action := "p2_down"
...
func _physics_process(delta):
	...
	var pad_dash := Input.is_joy_button_pressed(device_id, JOY_BUTTON_RIGHT_SHOULDER)
...
func apply_effect(buff: String, duration: float):
	match buff:
		"speed":
			...
			if SPEED > 1000:
				SPEED = 1000
			get_tree().call_group("main", "show_powerup_label", "\n\n    Player 2 Acquired Powerup : SPEED BOOST ")
		"jump":
			JUMP_VELOCITY -= 150
			if JUMP_VELOCITY < -1200:
				JUMP_VELOCITY = -1200
			get_tree().call_group("main", "show_powerup_label", "\n\n    Player 2 Acquired Powerup : JUMP BOOST ")
